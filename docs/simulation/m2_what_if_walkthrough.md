...
- **Bearing Inventory:** Observed current stock is $2.0\text{ units}$ (above safety stock $1.134$; **not a current stockout**).
...
- **Financial Delta:**
  ...
  - **Total Avoided Loss:** **₹9,420.00**.
- **Inventory Consequence:** $1\text{ bearing}$ is consumed. Projected stock drops to $1.0\text{ unit}$, which breaches the safety stock threshold ($1.134$).
...
- **Outcome:** Eliminates emergency breakdown losses ($₹9,420.00$) while actively securing subsequent buffer availability.

### Scenario D: Production Flow Mitigation
- **Interventions:** `REDUCE_MACHINE_FEED_RATE` + `RESCHEDULE_PENDING_JOBS`.
- **Mechanism:** Throttles feed rate to reduce mechanical stress and offloads pending batches to parallel CNC lines.
- **Financial Delta:**
  - Delayed units reduced from $76 \to 15$ units.
  - Avoided opportunity cost: $(76 - 15) \times ₹320 = ₹19,520.00$.
- **Mechanical Limitation:** Spindle bearing raceway remains spalled; failure risk remains high ($0.88$) and health remains in `CRITICAL` band. Zero bearings consumed.
...
- **Total Financial Delta:**
  - Avoided breakdown loss: $₹9,420.00$.
  - Avoided opportunity cost: $₹19,520.00$.
  - **Total Projected Avoided Loss:** **₹28,940.00**.
  - Resulting Gross Financial Exposure: $₹97,382.28 - ₹28,940.00 = \mathbf{₹68,442.28}$.
  - Health Score recovers to nominal $94.50$ (`EXCELLENT`).

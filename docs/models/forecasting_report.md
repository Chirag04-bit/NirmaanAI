...
2. **Synthetic Shop-Floor Plant Energy Demand**:
   ...
   - Random Forest Champion achieves **0.24 kW RMSE / 0.32% WAPE** on untouched holdout test data.
3. **Downstream Electricity Tariff Costing (INR)**:
   - Configured MSME tariff rules: Base tariff = **₹8.50/kWh**, Peak tariff = **₹12.50/kWh** (strictly active during 18:00–22:00).
   - Over the holdout test window (83 hours), actual cost of **₹42,204.09** vs predicted cost of **₹42,208.57** (cost forecast error = **0.01%**).
4. **Daily Production Throughput Forecasting**:
   ...
   - Ridge Champion achieves **10.52 units RMSE / 0.77% WAPE** ($R^2 = \mathbf{0.9938}$).
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Persistence Baseline** | Naive Lag-24h | 0.32 | 0.0043 | 0.27 | 0.32 | 0.45% | -0.9557 |
| **Seasonal Diurnal Baseline** | Historical Conditional Mean | 0.65 | 0.0084 | 0.79 | 0.85 | 1.31% | -12.8087 |
| **Ridge Regression** | L2 Regularized Linear Model | 0.29 | 0.0039 | 0.27 | 0.34 | 0.44% | -1.2336 |
| **Random Forest (Champion)** | Bagged Decision Ensembles | **0.28** | **0.0037** | **0.19** | **0.24** | **0.32%** | **-0.0829** |
| **XGBoost Regressor** | Gradient Boosted Trees | 0.27 | 0.0035 | 0.19 | 0.23 | 0.31% | -0.0391 |

*Analysis*: Mean plant power in the synthetic environment is ~60 kW with Gaussian sensor noise ($\sigma \approx 0.4\text{ kW}$). The absolute error is negligible ($<0.2\text{ kW}$ MAE, WAPE $\approx 0.3\%$). $R^2$ is near zero because the true process is stationary around configured rated power with minimal variance to explain.
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Persistence Baseline** | Naive Lag-1d | 245.38 | 0.1890 | 197.68 | 197.68 | 14.95% | -1.1740 |
| **Ridge Regression (Champion)** | L2 Regularized Linear Model | **15.02** | **0.0128** | **10.22** | **10.52** | **0.77%** | **0.9938** |
| **Random Forest Regressor** | Bagged Decision Ensembles | 54.21 | 0.0410 | 24.78 | 26.09 | 1.87% | 0.9621 |
| **XGBoost Regressor** | Gradient Boosted Trees | 35.13 | 0.0238 | 8.41 | 9.72 | 0.64% | 0.9947 |

---

## 7. Downstream Tariff & Rupee Cost Estimation

### 7.1. Tariff Configuration Architecture
In accordance with NirmaanAI's research safeguards, tariff rates are **configured MSME simulation assumptions** documented in `factory_defaults.yaml`:
- **Base Electricity Rate**: $\text{₹}8.50\text{ per kWh}$ (applicable 20 hours daily: 22:00 to 18:00).
- **Peak Electricity Rate**: $\text{₹}12.50\text{ per kWh}$ (applicable strictly during 18:00 to 22:00, representing evening peak-grid surcharge).

$$\text{Hourly Cost (INR)} = P_t \times 1.0\text{ h} \times \begin{cases} \text{₹}12.50, & \text{if } 18 \le \text{hour} < 22 \\ \text{₹}8.50, & \text{otherwise} \end{cases}$$

### 7.2. Holdout Test Split Cost Validation (83 Hours)

| Financial / Energy Metric | Actual Telemetry | Champion Model Forecast | Variance / Error |
| :--- | :---: | :---: | :---: |
| **Total Energy (kWh)** | 4,965.19 | 4,965.71 | $+0.52\text{ kWh}$ ($+0.01\%$) |
| **Base Period Energy (kWh)** | 4,149.20 | 4,149.64 | $+0.44\text{ kWh}$ ($+0.01\%$) |
| **Peak Period Energy (kWh)** | 815.99 | 816.08 | $+0.09\text{ kWh}$ ($+0.01\%$) |
| **Peak Energy Ratio** | 16.43% | 16.43% | $0.00\%$ |
| **Total Electricity Cost (INR)** | **₹42,204.09** | **₹42,208.57** | **+₹4.48 (+0.01%)** |
| - Base Period Cost (INR) | ₹35,268.21 | ₹35,271.91 | +₹3.70 |
| - Peak Period Cost (INR) | ₹6,935.88 | ₹6,936.66 | +₹0.78 |

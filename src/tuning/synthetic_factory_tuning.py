"""
NirmaanAI Synthetic Factory Anomaly Tracking Tuning & Validation
Task: Multi-Station Factory Health Index & Anomaly Detection (Stations M1–M5)

Priority 10 Tuning Module
- Preserves pre-decision cutoff (2026-01-21 12:00 UTC) on train.parquet
- MAINT_0003 strictly quarantined from prospective training inputs
- Mathematically defined pre-selection validation metric: Multi-Station Anomaly Separation Index (MASI)
  MASI = |mu_nominal - mu_val_tail| / (sigma_nominal + sigma_val_tail + eps)
  where val_tail is the bottom 10% anomaly score tail in validation.
- Bounded deterministic hyperparameter exploration across Isolation Forest, PCA Reconstruction, and Robust Z-score
- Model selection strictly on val.parquet via MASI
- Exactly ONE blind evaluation on test.parquet
- Emits artifacts to models/tuned/synthetic_factory/
"""
...
def compute_masi(train_scores: np.ndarray, val_scores: np.ndarray) -> float:
    """
    Computes Multi-Station Anomaly Separation Index (MASI) mathematically:
    MASI = |mu_nominal - mu_val_tail| / (sigma_nominal + sigma_val_tail + eps)
    ...
    eps = 1e-6
    masi = abs(mu_nom - mu_tail) / (sigma_nom + sigma_tail + eps)
    return float(masi)


class SyntheticFactoryTuning:
    ...
    def load_data(self):
        ...
        exclude = ["reading_id", "machine_id", "timestamp", "timestamp_dt", "is_pre_decision_cutoff"]
    ...
    def run_tuning(self, save_artifacts: bool = True) -> Dict[str, Any]:
        ...
        logger.info(f"[{self.DATASET_ID}] Starting bounded unsupervised tuning using pre-defined MASI metric...")
        ...
        logger.info(f"[{self.DATASET_ID}] Evaluating {len(candidates)} bounded configurations on validation factory telemetry...")

        val_results = []
        trained_models = {}

        # Evaluate baseline model on train & val to get baseline MASI
        ...
        base_masi_score = compute_masi(base_tr_scores, base_val_scores)
        base_test_scores = baseline_model.score_samples(data["X_test"])
        base_test_masi = compute_masi(base_tr_scores, base_test_scores)
        ...
        for name, model, family in candidates:
            ...
            masi = compute_masi(tr_scores, val_scores)
            metrics = {
                "candidate_name": name,
                "model_family": family,
                "masi": float(masi),
        ...
        # Select validation champion based on highest MASI
        ...
        # Check meaningful improvement: MASI delta > 0.05
        val_delta = tuned_val_score - base_masi_score
        is_accepted = val_delta > 0.05
        decision_status = "TUNED_MODEL_ACCEPTED" if is_accepted else "BASELINE_RETAINED"

        logger.info(f"[{self.DATASET_ID}] Champion: {champion_name} (Val MASI={tuned_val_score:.4f}, Baseline={base_masi_score:.4f}, Delta={val_delta:+.4f}) -> {decision_status}")
        ...
        tuned_test_masi = compute_masi(champ_tr_scores, test_scores)

        final_test_metrics = {
            "champion_model": champion_name,
            "primary_selection_metric": self.PRIMARY_METRIC,
            "test_masi": float(tuned_test_masi),
        ...
        improvement = compute_improvement(
            baseline_val=base_masi_score,
            tuned_val=tuned_val_score,
            baseline_test=base_test_masi,
            tuned_test=tuned_test_masi,
            higher_is_better=True,
        )
        improvement["decision_status"] = decision_status
        improvement["meaningful_improvement_threshold_masi"] = 0.05

        tuning_config = {
            "dataset_id": self.DATASET_ID,
            "primary_metric": self.PRIMARY_METRIC,
            "metric_definition": "Multi-Station Anomaly Separation Index (MASI) = |mu_nom - mu_tail| / (sigma_nom + sigma_tail + eps)",
        ...
        tuning_metadata = {
            "dataset_id": self.DATASET_ID,
            "task": "factory_telemetry_anomaly_tracking",
            "epistemic_status": "CONTROLLED_SYNTHETIC",
            "features_count": len(data["feature_cols"]),
            "quarantined_columns": ["MAINT_0003 (excluded from prospective training)"],
            ...
            "metric_name": "MASI",
            "baseline_val_score": base_masi_score,
            "tuned_val_score": tuned_val_score,
            "baseline_test_score": base_test_masi,
            "tuned_test_score": tuned_test_masi,
...
if __name__ == "__main__":
    tuner = SyntheticFactoryTuning()
    res = tuner.run_tuning()
    print("Synthetic Factory Tuning Complete:", res["champion"], res["improvement"])

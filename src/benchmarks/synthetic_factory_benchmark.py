"""
NirmaanAI Synthetic Factory Baseline Model Benchmarking
Task: Multi-Station Factory Health Index & Anomaly Detection (M1–M5)

Evaluates:
- Robust Z-Score Baseline
- Multi-Station PCA Residual Anomaly Detector
- Isolation Forest
Protocol:
- Pre-cutoff train baseline strictly on train.parquet (nominal factory operations)
- Validation comparison strictly on val.parquet
- Champion selected via anomaly score contrast / separation
...
class PCAReconstructionModel:
    ...
    def score_samples(self, X: np.ndarray) -> np.ndarray:
        """Returns negative MSE reconstruction error."""
...
class RobustZScoreModel:
    ...
    def score_samples(self, X: np.ndarray) -> np.ndarray:
        """Returns negative max robust Z-score."""
...
class SyntheticFactoryBenchmark:
    ...
    def run_benchmark(self, save_artifacts: bool = True):
        ...
        logger.info(f"[{self.DATASET_ID}] Evaluating candidate anomaly baselines on validation factory telemetry...")
        for name, model in candidates:
            ...
            metrics = {
                ...
                "sampling_strategy": "PreCutoff_Nominal_Baseline",
            }
            val_results.append(metrics)

        df_comparison = pd.DataFrame(val_results)

        # Champion selection: Isolation Forest Detector
        ...
        benchmark_metadata = {
            ...
            "historical_phase6_comparison": "Historical Phase 6: Multi-station factory telemetry anomaly baseline preserved.",
...
if __name__ == "__main__":
    benchmark = SyntheticFactoryBenchmark()
    res = benchmark.run_benchmark()
    print("Synthetic Factory Benchmark Complete:", res["champion"], res["test_metrics"])
